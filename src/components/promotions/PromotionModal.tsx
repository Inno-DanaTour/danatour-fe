...
const PromotionModal: React.FC<PromotionModalProps> = ({
  ...
  const isUsed =
    mode === "EDIT" && !!initialData && (initialData.usedCount || 0) > 0;
  ...
  const fetchTours = async () => {
    try {
      ...
      const response = await tourService.getTours(
        1,
        100,
        role === "COMPANY" ? companyId : undefined,
      );
  ...
  const validate = () => {
    ...
    if (
      ...
      Number(formData.usageLimit) < initialData.usedCount
    ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "EDIT" && initialData) {
        if (role === "ADMIN") {
          await promotionService.updateAdminPromotion(
            initialData.id,
            requestData,
          );
        } else {
          await promotionService.updateCompanyPromotion(
            initialData.id,
            requestData,
          );
        }
  ...
  const toggleTourSelection = (id: number) => {
    ...
    if (current.includes(id)) {
      setFormData({
        ...formData,
        tourIds: current.filter((tid) => tid !== id),
      });
    } else {
  ...
  const handleSelectAll = (visibleTours: TourListItem[]) => {
    ...
    if (allSelected) {
      ...
      setFormData({
        ...formData,
        tourIds: currentIds.filter((id) => !allVisibleIds.includes(id)),
      });
    } else {
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div
                        className={`w-2 h-2 rounded-full ${step === 1 ? "bg-primary" : "bg-gray-200"}`}
                      />
                      <div
                        className={`w-2 h-2 rounded-full ${step === 2 ? "bg-primary" : "bg-gray-200"}`}
                      />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        />
                        {errors.code && (
                          <p className="text-red-500 text-[10px] font-black mt-2 ml-1 uppercase">
                            {errors.code}
                          </p>
                        )}
                      </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                discountValue: e.target.value as any,
                              })
                            }
                            disabled={isUsed}
                          />
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  maxDiscountAmount: e.target.value as any,
                                })
                              }
                              disabled={isUsed}
                            />
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  usageLimit: e.target.value as any,
                                })
                              }
                            />
                          </div>
                        ...
                        {availableTours.length > 0 && (
                          ...
                          >
                            {availableTours
                              ...
                              .every((t) => formData.tourIds?.includes(t.id))
                              ? "Deselect All"
                              : "Select All"}
                          </button>
                        )}
                      ...
                      {toursLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {availableTours
                            ...
                            .map((tour) => {
                              const isSelected = formData.tourIds?.includes(
                                tour.id,
                              );
                              return (
                                <button
                    ...
                    {role === "COMPANY" && formData.tourIds?.length === 0 && (
                      <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex gap-3 text-amber-700">
                        ...
                        <p className="text-xs font-bold leading-relaxed">
                          Please select at least one tour. Campaigns from
                          providers must be linked to specific offerings.
                        </p>
                      </div>
                    )}
                  </div>
