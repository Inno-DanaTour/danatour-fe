        ...
        >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">
                  {loading ? "Loading Tour Details..." : tour?.title || "Tour Detail"}
                </h2>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
            {loading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-slate-500 font-bold mb-6 max-w-xs">{error}</p>
                <button
                   onClick={handleRetry}
                ...
                <div className="lg:col-span-12 space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-col justify-between group hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Category</span>
                       <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center">
                           <Info size={18} />
                         </div>
                         <span className="text-lg font-black text-slate-900">{tour.category.name}</span>
                       </div>
                    </div>
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-col justify-between group hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Market Node</span>
                       <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center">
                           <MapPin size={18} />
                         </div>
                         <span className="text-lg font-black text-slate-900">{tour.place.name}</span>
                       </div>
                    </div>
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-col justify-between group hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Duration Cycle</span>
                       <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center">
                           <Clock size={18} />
                         </div>
                         <span className="text-lg font-black text-slate-900">{tour.durationDays}D / {tour.durationNights}N</span>
                       </div>
                    </div>
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-col justify-between group hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 text-primary">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Adulte Rate</span>
                       <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold">
                           <DollarSign size={18} />
                         </div>
                         <span className="text-2xl font-black">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(tour.adultPrice)}</span>
                       </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center bg-slate-100/50 p-1.5 rounded-2xl w-fit">
                    {(['overview', 'itinerary', 'schedules'] as const).map((tab) => (
                      <button
                        key={tab}
                    ...
                    {activeTab === 'overview' && (
                       ...
                       >
                         <div className="lg:col-span-7 space-y-8">
                           <div>
                             <h4 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Description Intel</h4>
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              {tour.images.map((img, i) => (
                                <div key={i} className="aspect-[16/10] rounded-3xl overflow-hidden border-2 border-white shadow-xl group cursor-pointer relative">
                                   <img src={img.imageUrl} alt="" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                   {img.isThumbnail && (
                                     <span className="absolute top-4 left-4 bg-primary text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full shadow-lg">Primary</span>
                                   )}
                                </div>
                              ))}
                           </div>
                         ...
                         <div className="lg:col-span-5 space-y-6">
                            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-slate-900/20">
                               ...
                               <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/10">
                                  ...
                                  <div>
                                    <p className="font-black text-lg leading-tight">Entity Name ID: {tour.companyId}</p>
                                    <p className="text-white/50 text-xs font-bold mt-1">Status: Operational</p>
                                  </div>
                               </div>
                            </div>

                            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 space-y-6">
                               <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Ecosystem Metrics</h4>
                               <div className="space-y-4">
                                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                                    <span className="text-sm font-bold text-slate-500">Quality Index</span>
                                    <div className="flex items-center gap-2 text-primary">
                                      <Star size={16} fill="currentColor" />
                                      <span className="font-black">{tour.averageRating || 0}</span>
                                    </div>
                                  </div>
                                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                                    <span className="text-sm font-bold text-slate-500">Signal Count</span>
                                    <span className="font-black text-slate-900">{tour.reviewCount || 0} Feedbacks</span>
                                  </div>
                                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                                    <span className="text-sm font-bold text-slate-500">System Status</span>
                                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                                      tour.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 
                                      tour.status === 'PENDING' ? 'bg-amber-50 text-amber-600 border-amber-100' : 
                              ...
                              <div key={i} className="relative group">
                                ...
                                <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 group-hover:bg-white group-hover:shadow-xl group-hover:shadow-slate-200/50 transition-all duration-300">
                                   <div className="flex items-center gap-4 mb-4">
                                      <span className="text-xs font-black bg-primary text-white px-3 py-1 rounded-full uppercase tracking-widest shadow-lg shadow-primary/20">Day {step.day}</span>
                       ...
                       >
                         {tour.schedules && tour.schedules.length > 0 ? (
                           tour.schedules.map((s) => (
                             <div key={s.id} className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 flex items-center justify-between group hover:bg-white hover:shadow-xl transition-all duration-300">
                               <div className="flex items-center gap-6">
                                 ...
                                 <div>
                                   <p className="font-black text-slate-900 text-lg tracking-tight">System Node Departure</p>
                               ...
                               <div className="text-right">
                                  <div className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border ${
                                    s.availableSlots > 5 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'
                                  }`}>
                           ...
                           <div className="col-span-full py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-100 flex flex-col items-center gap-4">
                              ...
                              <p className="text-slate-400 font-black text-xs uppercase tracking-widest">No Scheduled Nodes Found</p>
                           </div>
                         )}
                       </motion.div>
                    )}
          ...
          <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
             <div className="flex items-center gap-4">
               ...
               <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Signal Stable | v2.0.4-LTS</span>
             </div>
             <button
               onClick={onClose}
