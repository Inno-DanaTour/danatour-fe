    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: "Active Promos", value: promotions.filter(p => p.isActive).length, icon: Tag, color: "bg-green-500" },
                    { label: "Total Usage", value: promotions.reduce((acc, p) => acc + (p.usedCount || 0), 0), icon: Users, color: "bg-blue-500" },
                    { label: "Expiring Soon", value: "3", icon: Clock, color: "bg-amber-500" },
                ].map((stat, i) => (
                ...
                <div className="flex bg-gray-50 p-1.5 rounded-2xl gap-1">
                    {(["ALL", "PLATFORM", "PROVIDER"] as const).map((type) => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${sponsorFilter === type
                        ...
                        >
                            <div className="p-8">
                                <div className="flex justify-between items-start mb-6">
                                    <div className={`p-3 rounded-2xl ${promo.discountType === 'PERCENTAGE' ? 'bg-indigo-50 text-indigo-600' : 'bg-green-50 text-green-600'}`}>
                                        {promo.discountType === 'PERCENTAGE' ? <Percent size={24} /> : <CreditCard size={24} />}
                                    </div>
                                    <div className="flex gap-2">
                                        ...
                                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${promo.sponsorType === 'PLATFORM' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'}`}>
                                            {promo.sponsorType}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="mb-6">
                                    <h3 className="text-xs font-black uppercase tracking-tighter text-gray-400 mb-1">PROMO CODE</h3>
                                    <div className="flex items-center gap-2">
                                        <span className="text-2xl font-black tracking-widest text-primary font-mono">{promo.code}</span>
                                    </div>
                                </div>

                                <div className="space-y-4">
                                    <div>
                                        <p className="text-xl font-black text-gray-900">{promo.title || "Flash Sale Campaign"}</p>
