        ...
        <div className="min-h-screen bg-background pb-20">
            ...
            <main className="pt-24 md:pt-32 px-4 md:px-6 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                    {[
                        { label: "Active Codes", value: promotions.filter(p => p.isActive).length, icon: Tag, color: "bg-primary" },
                        { label: "Redemptions", value: promotions.reduce((acc, p) => acc + (p.usedCount || 0), 0), icon: TrendingUp, color: "bg-cta" },
                        { label: "Applied Tours", value: "8", icon: Package, color: "bg-black" },
                        { label: "Revenue Saved", value: "2.5M", icon: Calendar, color: "bg-blue-500" },
                    ].map((stat, i) => (
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20">
                        ...
                        <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">Fetching your campaigns...</p>
                    </div>
                ) : promotions.length === 0 ? (
                    <div className="bg-gray-50 p-20 rounded-[3rem] text-center border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-gray-500 mb-8 max-w-md mx-auto">
                            Boost your booking rates by creating limited-time discount codes for your customers.
                        </p>
                        <button className="btn-primary py-4 px-10">Create First Coupon</button>
                    </div>
                ) : (
                            ...
                            >
                                <div className="p-8 flex-1">
                                    <div className="flex justify-between items-center mb-6">
                                        <div className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${promo.discountType === 'PERCENTAGE' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'}`}>
                                            {promo.discountType === 'PERCENTAGE' ? 'Percentage' : 'Fixed Amount'}
                                        </div>
                                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${promo.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                            {promo.isActive ? 'Active' : 'Inactive'}
                                        </span>
                                    </div>

                                    <h3 className="text-3xl font-black text-primary font-mono tracking-widest mb-2">{promo.code}</h3>
                                    <p className="text-lg font-black text-gray-900 mb-4">{promo.title || "Seasonal Discount"}</p>
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-2xl space-y-2 mb-6">
                                        ...
                                        {promo.maxDiscountAmount && (
                                            <div className="flex justify-between items-center text-sm">
                                                ...
                                                <span className="font-black">{promo.maxDiscountAmount.toLocaleString()} VND</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center pt-4 border-t border-gray-50">
                                        <div className="flex items-center gap-2 text-gray-400 font-bold text-[10px] uppercase tracking-widest">
                                            <Calendar size={14} /> {new Date(promo.validTo).toLocaleDateString()}
                                        </div>
