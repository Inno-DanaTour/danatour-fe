...
export const tourService = {
  getTours: async (
    ...
    size: number = 10,
    companyId?: number,
  ): Promise<PagedResponse<TourListItem>> => {
    let url = `/tours?page=${page - 1}&size=${size}`;
    if (companyId) {
      url += `&companyId=${companyId}`;
    }
    const response =
      await api.get<ApiResponse<PagedResponse<TourListItem>>>(url);
  ...
  createTour: async (formData: FormData): Promise<TourDetail> => {
    const response = await api.postMultipart<ApiResponse<TourDetail>>(
      "/tours",
      formData,
    );
    return response.data;
  ...
  updateTour: async (
    ...
    const response = await api.putMultipart<ApiResponse<TourDetail>>(
      `/tours/${id}`,
      formData,
    );
    return response.data;
